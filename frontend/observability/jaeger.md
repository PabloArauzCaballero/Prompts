...
No modificar contratos con el backend sin revisar el impacto.
No eliminar lógica funcional por razones visuales.
No crear una interfaz bonita pero difícil de utilizar.
No usar componentes diferentes para resolver el mismo patrón.
No duplicar formularios, tablas, modales, filtros o notificaciones.
No crear componentes excesivamente grandes.
No crear archivos de más de 300 líneas salvo justificación técnica.
No utilizar any de forma indiscriminada.
No ocultar errores de TypeScript.
No desactivar reglas de lint para evitar corregir problemas.
No introducir dependencias sin justificar su necesidad.
No utilizar estilos inline salvo casos estrictamente necesarios.
No usar datos simulados para ocultar integraciones incompletas.
No introducir animaciones que afecten el rendimiento.
No convertir todos los componentes en Client Components.
No utilizar useEffect para lógica que pueda resolverse de forma declarativa.
No realizar solicitudes duplicadas.
No almacenar datos sensibles en el navegador.
No exponer variables privadas mediante prefijos públicos.
No declarar una fase como completada sin validar sus criterios.
Documentar toda decisión arquitectónica importante.
Priorizar claridad, consistencia, rendimiento y mantenibilidad.
Mantener compatibilidad completa con Turbopack.
Preservar el comportamiento funcional actual salvo correcciones justificadas.
...
Define los archivos afectados.
Establece criterios de aceptación.
Identifica riesgos de regresión.
Define cómo se validarán los cambios.
...
Actualiza el registro de riesgos.
Registra archivos creados y modificados.
Comprueba que no existan regresiones.
...
Ejecutar la aplicación sin modificaciones.
Ejecutar:
Build.
Lint.
Type check.
...
Capturar evidencia de las pantallas principales.
Crear una línea base visual y técnica.
Identificar los flujos críticos del sistema.
Entregables
Informe inicial.
...
Identificar dependencias incompatibles.
Eliminar configuraciones heredadas innecesarias.
Revisar:
Aliases.
Imports.
CSS.
...
Revisar errores de hot reload.
Revisar reconstrucciones innecesarias.
Revisar tiempos de arranque.
Revisar tiempos de actualización.
Verificar compatibilidad en desarrollo y producción.
No mantener una configuración paralela de Webpack salvo necesidad real.
Restricciones
...
Actividades
Identificar formularios repetidos.
Centralizar schemas.
Validar en cliente sin reemplazar la validación del backend.
Traducir errores del servidor.
Implementar mensajes específicos.
Manejar errores por campo y globales.
Evitar pérdida accidental de cambios.
Implementar confirmación de salida cuando sea necesaria.
Manejar estados:
Inicial.
Modificado.
Enviando.
...
Mantener valores al fallar.
Implementar máscaras solamente cuando aporten valor.
...
El proyecto inicie correctamente con Turbopack.
El build de producción finalice sin errores.
TypeScript no presente errores.
El linter no presente errores críticos.
No existan errores de hidratación.
No existan errores críticos en consola.
Todas las rutas principales funcionen.
La autenticación funcione correctamente.
Los permisos se respeten.
Los formularios validen correctamente.
Los errores del backend sean manejados.
Las tablas funcionen.
Los filtros funcionen.
La paginación funcione.
Los modales sean accesibles.
Los estados de carga estén implementados.
Los estados vacíos estén implementados.
Los estados de error estén implementados.
El frontend sea responsive.
Los flujos críticos funcionen en móvil.
La navegación por teclado sea posible.
El contraste sea adecuado.
Las pruebas unitarias pasen.
Las pruebas de integración pasen.
Las pruebas E2E pasen.
Las pruebas visuales no presenten regresiones.
El rendimiento haya sido medido.
El bundle haya sido analizado.
No se expongan secretos.
No existan dependencias innecesarias.
No existan componentes duplicados.
Exista un sistema de diseño documentado.
Exista documentación técnica.
Exista evidencia antes y después.
No queden pendientes evitables.
...
Diseñar el plan de refactorización.
...
Ejecutar pruebas.
Ejecutar el build con Turbopack.
Documentar decisiones.
Comparar antes y después.
Eliminar código obsoleto.
Realizar una revisión integral final.
